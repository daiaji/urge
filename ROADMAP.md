...
| 功能 | 引擎提供什么 | 状态 |
|------|-------------|:----:|
| **鼠标输入 API**（坐标/滚轮/按钮） | SDL 原始事件数据暴露为 Ruby 常量 | 待实现 |
| **剪贴板访问** | SDL 剪贴板 API 绑定 | 待实现 |
| **CFG 模块**（运行时配置读写） | 用户目录路径 + JSON 文件 I/O 暴露 | 待实现 |
| **System 平台检测** | SDL/预编译宏的字符串常量暴露 | 待实现 |
| **System.mount / unmount**（mod 支持） | PhysFS 挂载点动态增删 API | 待实现 |
| **Graphics.screenshot** | `snap_to_bitmap` → `to_file` PNG 导出链 | 待实现 |
| **Graphics API 扩展**（resize / center / scale） | SDL 窗口操作 + GPU 缩放参数绑定 | 待实现 |
| **扩展键盘输入**（扫描码 / 文本输入） | SDL 扫描码常量 + IME 缓冲区暴露 | 待实现 |
...
| MiniFFI / Win32API 兼容 | `class Win32API; def call(*); end; end` 三行 Ruby |
