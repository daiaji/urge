...
void PSMain(in PSInput PSIn, out PSOutput PSOut) {
  ...
  float2 uv = PSIn.UV;
  {
      ...
      {
          float4 _61 = u_Texture1.Sample(u_Texture1_sampler, uv + (float2(float(_26 - 2), 0.0f) * u_InputPt));
          float _37 = _11(_61);
          _23 = max(_37, _23);
      }
      PSOut.Color = _23;
...
void PSMain(in PSInput PSIn, out PSOutput PSOut) {
  ...
  float2 uv = PSIn.UV;
  {
      ...
      float _23 = _11(_34);
      float _36 = min(_23, u_Texture2.Sample(u_Texture2_sampler, uv).x);
