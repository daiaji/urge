...
| `FontSubs` | — | 字体替换表，`"From>To"` 逗号分隔。`LoadFontInternal` 中按原名查找替换 |
...
- **`require_relative` 不可用问题**：`rgss3_patch.rb` 原用 `require_relative` 加载 `rgss3_compat.rb`，但 RGSS 脚本通过 `eval()` 执行无文件系统上下文。已移除 `require_relative`，改为通过 `rm-toolkit --inject-script` 分别注入两个补丁文件。
- **textSize 精度**：追加空格再减宽度，补偿 FreeType 末字偏大问题（`canvas_impl.cc` / `surface_impl.cc`）。
- **透明文本+轮廓 alpha**：移除 `font_color.a = outline_color.a = 255` 强制不透明，TTF 渲染使用实际 alpha（`font_impl.cc`）。
- **对齐 outline 偏移**：Center/Right 对齐补偿 outline 边框宽度（`canvas_impl.cc`）。
- **阴影颜色**：阴影 RGB 取自文本颜色而非纯黑，与 mkxp-z 行为一致（`font_impl.cc`）。
...
| 子模块 | 说明 | 状态 |
|------|------|:----:|
| `Input::Controller.connected?` | 手柄连接状态 | 待实现 |
| `Input::Controller.name` | 手柄名称 | 待实现 |
| `Input::Controller.axes_left` / `axes_right` | 左/右摇杆原始值 | 待实现 |
| `Input::Controller.raw_button_states` / `raw_axes` | 原始按键/轴数据数组 | 待实现 |
| `Input::Controller.power_level` | 手柄电量 | 待实现 |
