...
void PSMain(in PSInput PSIn, out PSOutput PSOut) {
  ...
  uint2 sz = uint2(u_InputSize);
  if (gxy.x >= sz.x || gxy.y >= sz.y) return;
  float2 pt = u_InputPt;
...
//!NUM_THREADS 64
  ...
  if (sub_idx == 0)      cnn_rgb = float3(r0.x, r1.x, r2.x);
  else if (sub_idx == 1) cnn_rgb = float3(r0.y, r1.y, r2.y);
  else if (sub_idx == 2) cnn_rgb = float3(r0.z, r1.z, r2.z);
  else                   cnn_rgb = float3(r0.w, r1.w, r2.w);
  float3 residual = u_Texture.SampleLevel(u_Texture_sampler, PSIn.UV, 0).rgb;
  PSOut.Color = float4(saturate(residual + cnn_rgb), 1.0);
}
)";
// 38490 chars
