...
| **Anime4K Denoise L / CuNNy 2x** | UDL 4-pass compute + CuNNy-4x16/4x24 NVL 6-pass compute，按需 lazy loading |
| **2x Auto-fit Window** | UDL/CuNNy 模式自动锁定 `resolution * 2` 窗口；按窗口外框居中，避免模型切换时标题栏偏移 |
...
当前模式映射：0=Bilinear / 1=Nearest / 2=Lanczos3 / 3=Bicubic / 4=Anime4K / 5=Anime4K+Sobel / 6=Anime4K Denoise L / 7=CuNNy-4x16-NVL / 8=CuNNy-4x24-NVL

```ruby
Graphics.scaling_mode          # => 0~8
...
| **ME/BGM 状态机** | `MeWatchState` | ME 播放期间 BGM 200ms 淡出、暂停；ME 结束后 1000ms 淡入恢复，对齐 mkxp-z 单轨语义 | ✅ |
...
| **绑定表清理** | 移除多余的第 4 列 |
| **F1 设置菜单键盘捕获** | 设置/控制台 overlay 捕获键盘时阻止游戏 Input 同时消费，避免菜单和游戏光标双重移动 |
...
| **Anime4K Denoise L / CuNNy 2x 超分** | UDL 4-pass + CuNNy-4x16/4x24 NVL compute pipeline，lazy loading + bytecode cache |
| **2x Auto-fit Window 外框居中** | UDL/CuNNy 自动适配窗口时按 SDL window border 计算外框居中，避免切换模型时窗口上移 |
| **CAS 锐化** | AMD FidelityFX CAS 5-tap 十字形，sharpness 滑块 0~1 |
| **Sobel 自适应混合 + 细调** | Anime4K Enhance PS 内联 Sobel 3×3，Warp/Dark 三滑块 |
| **缩放算法 Ruby API** | `Graphics.scaling_mode` auto-gen 绑定（0~8） |
| **设置持久化 + UI 统一** | 全量 INI 写回 SaveConfigure()，帧尾 auto-save + 析构 fallback，三区 Reset + 全局 Reset All |
| **ME/BGM 淡出淡入状态机** | ME 播放期间 BGM duck 到 0 并暂停；ME 结束后恢复并淡入，脚本在 ME 期间 play/stop/fade BGM 的边界按 mkxp-z 语义处理 |
